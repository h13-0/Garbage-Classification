#include "Intel8080.h"
#include "ST7789V_240x240_Intel8080.h"
...
*/
    //DBL=DataH;
    I8080_Writebyte(DataH);
    WR=0;
    WR=1;

    //DBL=DataL;
    I8080_Writebyte(DataL);
    WR=0;
    WR=1;
...
void WriteOneDot(unsigned int color)
{
    ...
    //DBL=color>>8;
    I8080_Writebyte(color>>8);
    WR=0;
    WR=1;

    //DBL=color;
    I8080_Writebyte(color&0xff);
    WR=0;
    WR=1;
...
//---------------------------------ST7789V Power setting--------------------------------------//
    ...
    WriteData(0x20);
    WriteComm(0xc6); //帧率
    WriteData(0x01); //00 -> 119hz -> Boom; 01 -> 111hz; C6 -> 60hz;
...
void DispBand(void)
{
    ...
    for(i=0; i<8; i++)
    {
        for(j=0; j<(ROW/8); j++)
        {
            for(k=0; k<COL; k++)
            {
                I8080_Writebyte(color[i]>>8);
                WR=0;
                WR=1;

                I8080_Writebyte(color[i]&0xff);
                WR=0;
                WR=1;
            }
        }
    ...
    for(j=0; j<(ROW%8); j++)
    {
        for(k=0; k<COL; k++)
        {
            I8080_Writebyte(color[7]>>8);
            WR=0;
						waitnms;
            WR=1;

            I8080_Writebyte(color[7]);
            WR=0;
						waitnms;
            WR=1;
        }
...
void DispScaleHor1(void)
{
    ...
    for(k=0; k<ROW/4; k++)
    {
        ...
        for(i=0; i<32; i++)
        {
            for(j=0; j<COL/32; j++)
            {
                I8080_Writebyte((i*2)>>3);
                WR=0;
                WR=1;

                I8080_Writebyte((i*2)<<5);
                WR=0;
                WR=1;
            }
        }
    ...
    for(k=0; k<(ROW/4+ROW%4); k++)
    {
        ...
        for(i=0; i<32; i++)
        {
            for(j=0; j<COL/32; j++)
            {
                I8080_Writebyte((i<<3)|((i*2)>>3));
                WR=0;
                WR=1;

                I8080_Writebyte(((i*2)<<5)|i);
                WR=0;
                WR=1;
            }
        }
...
void DispScaleVer(void)
{
    ...
    for(k=0; k<32; k++)
    {
        ...
        for(i=0; i<ROW/32; i++)
        {
            ...
            for(j=0; j<COL/4; j++)
            {
                I8080_Writebyte((k*2)>>3);
                WR=0;
                WR=1;

                I8080_Writebyte((k*2)<<5);
                WR=0;
                WR=1;
            }
            // BLUE
            ...
            for(j=0; j<COL/4; j++)
            {
                I8080_Writebyte((k<<3)|((k*2)>>3));
                WR=0;
                WR=1;

                I8080_Writebyte(((k*2)<<5)|k);
                WR=0;
                WR=1;
            }
...
*/
        for(j=0; j<16; j++)
        {
            for(k=0; k<COL/16; k++)
            {
                I8080_Writebyte(((j*2)<<3)|((j*4)>>3));
                WR=0;
                WR=1;

                I8080_Writebyte(((j*4)<<5)|(j*2));
                WR=0;
                WR=1;
            }
        }
...
void DispGrayHor32(void)
{
    ...
    for(i=0; i<ROW; i++)
    {
        ...
        for(j=0; j<32; j++)
        {
            for(k=0; k<COL/32; k++)
            {
                I8080_Writebyte((j<<3)|((j*2)>>3));
                WR=0;
                WR=1;

                I8080_Writebyte(((j*2)<<5)|j);
                WR=0;
                WR=1;
            }
        }
...
void DispBlock(void)
{
    ...
    for(i=0; i<k; i++)
    {
        for(j=0; j<COL; j++)
        {
            I8080_Writebyte(GRAY50>>8);
            WR=0;
            WR=1;

            I8080_Writebyte(GRAY50&0xff);
            WR=0;
            WR=1;
        }
    }
    ...
    for(i=0; i<k*2; i++)
    {
        ...
        for(j=0; j<COL/2; j++)
        {
            I8080_Writebyte(BLACK>>8);
            WR=0;
            WR=1;

            I8080_Writebyte(BLACK);
            WR=0;
            WR=1;
        }
...
void EnterSLP(void)
{
    ...
    WriteComm(0x0007);
    WriteData(0x0131); // Set D1=0, D0=1
    Delay(10);
    WriteComm(0x0007);
    WriteData(0x0130); // Set D1=0, D0=0
    Delay(10);
    WriteComm(0x0007);
    WriteData(0x0000); // display OFF
    //************* Power OFF sequence **************//
    WriteComm(0x0010);
    WriteData(0x0080); // SAP, BT[3:0], APE, AP, DSTB, SLP
    WriteComm(0x0011);
    WriteData(0x0000); // DC1[2:0], DC0[2:0], VC[2:0]
    WriteComm(0x0012);
    WriteData(0x0000); // VREG1OUT voltage
    WriteComm(0x0013);
    WriteData(0x0000); // VDV[4:0] for VCOM amplitude
    Delay(200);                          // Dis-charge capacitor power voltage
    WriteComm(0x0010);
    WriteData(0x0082); // SAP, BT[3:0], APE, AP, DSTB, SLP
}
...
void ExitSLP(void)
{
    ...
    //Power On sequence
    WriteComm(0x0010);
    WriteData(0x0080); // SAP, BT[3:0], AP, DSTB, SLP
    WriteComm(0x0011);
    WriteData(0x0000); // DC1[2:0], DC0[2:0], VC[2:0]
    WriteComm(0x0012);
    WriteData(0x0000); // VREG1OUT voltage
    ...
    Delay(200);                          // Dis-charge capacitor power voltage
    WriteComm(0x0010);
    WriteData(0x1490); // SAP, BT[3:0], AP, DSTB, SLP, STB
    WriteComm(0x0011);
    WriteData(0x0227); // R11h=0x0221 at VCI=3.3V DC1[2:0], DC0[2:0], VC[2:0]
    Delay(50);                           // Delay 50ms
    WriteComm(0x0012);
    WriteData(0x009B); // External reference voltage =Vci;
    Delay(50);                           // Delay 50ms
    WriteComm(0x0013);
    WriteData(0x1800); // R13h=0x1200 when R12=009D VDV[4:0] for VCOM amplitude
    WriteComm(0x0029);
    WriteData(0x0027); // R29h=0x000C when R12=009D VCM[5:0] for VCOMH
    Delay(50);                           // Delay 50ms
    WriteComm(0x0007);
    WriteData(0x0133); // 262K color and display ON
}
